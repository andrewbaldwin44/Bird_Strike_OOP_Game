class Player {
  ...
  changeImageDirection() {
    if (keysPressed['ArrowRight']) {
      if (!this.invincible) this.setImage(PLAYER_IMAGE_HAPPY);
      this.domElement.style.transform = 'scale(-1, 1)';
    }
    else if (keysPressed['ArrowLeft']) {
      if (!this.invincible) this.setImage(PLAYER_IMAGE_DEFAULT);
  ...
  toggleInvincibility() {
    ...
    this.setImage(PLAYER_IMAGE_SAD);
  ...
  removeDestroyedBullets() {
    this.bullets.filter(bullet => !bullet.destroyed);
  }

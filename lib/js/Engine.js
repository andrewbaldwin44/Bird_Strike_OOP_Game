class Engine {
  ...
  spawnNewEnemy() {
    ...
    let [type, enemy_speed, enemy_image] = getEnemyType();

    let newEnemy = new Enemy(enemySpot, enemy_image, enemy_speed);

    if (type == 2) newEnemy.shootBullets();
  ...
  isBulletHitPlayer(enemy) {
    return enemy.bullets.some(bullet => {
      return isCollision(bullet, this.player,
